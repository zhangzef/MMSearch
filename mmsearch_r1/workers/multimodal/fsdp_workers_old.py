...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if role == 'actor':
            ...
            from mmsearch_r1.utils.torch_functional import get_sigmoid_decay_schedule
            ...
            lr_sigmoid_decay = optim_config.get('lr_sigmoid_decay_warmup', False)
            if lr_sigmoid_decay:
                lr_sigmoid_decay_ratio = optim_config.get('lr_sigmoid_decay_ratio', 0)  # by defualt will not decay
                num_warmup_steps = optim_config.get(
                    'lr_sigmoid_decay_warmup_steps', 60
                )  # by defualt lr stay high for around 60 steps
                print(f'[MMSearch-R1 LR Sigmoid Decay] {lr_sigmoid_decay_ratio=} {num_warmup_steps=}')
                actor_lr_scheduler = get_sigmoid_decay_schedule(
                    optimizer=actor_optimizer, lr_low=1 - lr_sigmoid_decay_ratio, num_warmup_steps=num_warmup_steps
                )
        else:
    ...
    def init_model(self):
        from mmsearch_r1.workers.multimodal.actor.dp_actor import MmDataParallelPPOActor
        ...
        if self._is_actor:
            ...
            self.actor = MmDataParallelPPOActor(
        ...
        if self._is_ref:
            ...
            self.ref_policy = MmDataParallelPPOActor(config=self.config.ref, actor_module=self.ref_module_fsdp)
